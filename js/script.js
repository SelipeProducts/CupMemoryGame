function getRandomInt(min, max) {
			...
			var score;
			var backup = localStorage.getItem("GameScore");
			if(score!=null){
				score=backup;
			}
			else if(score == null){
			...
			function clickImg(img){  //click img to see under
				...
				var clicked = checkIfClicked(currentCoord);
				...
				storeClickLocation(currentCoord);
			}
			...
			function checkIfClicked(current){  //returns true if already clicked
				var iIfClicked;
				var arraylength = storedLocation.length;
				for(iIfClicked=0; iIfClicked<arraylength; iIfClicked++){
					if(current[0]==storedLocation[iIfClicked] && current[1]==storedLocation[iIfClicked+1]){
						var c = alert("Coord Match");
						return true;
					}
				}
				return false;
			}
			function storeClickLocation(clicked){
				var clickedX = clicked[0];
				var clickedY = clicked[1];

				storedLocation.push(clickedX);
				storedLocation.push(clickedY);
